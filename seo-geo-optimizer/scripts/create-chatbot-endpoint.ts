 ...
 * Verifica se esiste giÃ  un endpoint chatbot o ne crea uno
 */
...
async function testOpenAI(apiKey: string) {
  ...
  try {
    ...
    if (data.error) {
      console.log(`   âŒ Errore: ${data.error.message}`);
      return false;
    }

    console.log(`   âœ… Funziona! Risposta: "${data.choices[0].message.content}"`);
    console.log(`   Modello usato: ${data.model}`);
    return true;
  } catch (e: any) {
    console.log(`   âŒ Errore connessione: ${e.message}`);
    return false;
  }
}
...
async function main() {
  ...
  console.log('ðŸ¤– LAPA - Setup Chatbot AI Professionale');
  console.log('â•'.repeat(80));
  console.log('');
  ...
  try {
    ...
    console.log('ðŸ”‘ Recupero API Key OpenAI da Odoo...');
    ...
    console.log(`   âœ… API Key trovata: ${apiKey.substring(0, 15)}...${apiKey.substring(apiKey.length - 5)}`);
    ...
    if (apiWorks) {
      console.log('\n' + 'â•'.repeat(80));
      console.log('âœ… API OpenAI FUNZIONANTE!');
      console.log('â•'.repeat(80));
      console.log(`
...
`);
    ...
    // (in produzione questo sarÃ  nel backend)
    console.log('\nðŸ“ Preparazione configurazione chatbot...');
    ...
    if (fs.existsSync(knowledgePath)) {
      ...
      console.log('   âœ… Knowledge base caricata');
    }
...
6. Rispondi SEMPRE nella lingua usata dal cliente`
    ...
    console.log(`   âœ… Configurazione salvata in: ${configPath}`);
