/**
 * Generate articles 55-60 for LAPA blog
 */
...
async function translateArticle(italianContent: any, targetLang: string, articleNumber: number): Promise<any> {
  console.log(`   🌍 Translating article ${articleNumber} to ${targetLang}...`);
...
async function main() {
  console.log('🚀 GENERATING ARTICLES 55-60 FOR LAPA BLOG\n');

  for (const article of ARTICLES) {
    const filename = `data/new-articles-2025/article-${String(article.number).padStart(2, '0')}-${article.id}.json`;

    console.log(`\n[${article.number}/60] ====== ${article.topic} ======`);

    try {
      // 1. Generate Italian article
      const italianContent = await generateItalianArticle(article);

      // 2. Translate to German, French, English
      const germanContent = await translateArticle(italianContent, 'de_DE', article.number);
      const frenchContent = await translateArticle(italianContent, 'fr_FR', article.number);
      const englishContent = await translateArticle(italianContent, 'en_US', article.number);

      // 3. Create full article object
      const fullArticle = {
  ...
  console.log('\n\n✅ ====== GENERATION COMPLETE ======');
  console.log('Generated: 6 articles (24 total with all languages)');
  console.log('📂 Files saved in: data/new-articles-2025/');
}
