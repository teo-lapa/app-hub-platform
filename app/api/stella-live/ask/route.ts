/**
 * STELLA LIVE - Dispatch al cervello vero (bridge claude -p su PC STELLA).
 * Chiamata dal tool `chiedi_a_stella` del modello Realtime. Riusa lo STESSO
 * bridge /ask di /stella-voce (nessuna modifica su STELLA). Niente STT/TTS:
 * la voce la fa il Realtime. Ritorna testo pulito + eventuali immagini/PDF.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!BRIDGE_URL || !BRIDGE_TOKEN) {
      return NextResponse.json({ error: 'Stella non e online (bridge non registrato).' }, { status: 503 });
    }
    ...
    if (!r.ok) {
      ...
      return NextResponse.json({ error: `Stella non risponde (${r.status}): ${t.slice(0, 160)}` }, { status: 502 });
    }
