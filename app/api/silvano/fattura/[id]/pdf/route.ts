...
/**
 * GET /api/silvano/fattura/[id]/pdf
 * Scarica il PDF della fattura (report nativo account.move, con QR-bill CH).
 */
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const url = `${ODOO_URL}/report/pdf/account.report_invoice/${id}`;
    ...
    return new NextResponse(buf, {
      headers: {
        ...
        'Content-Disposition': `inline; filename="fattura-${id}.pdf"`,
      },
    });
  } catch (error: any) {
    console.error('💥 [SILVANO/fattura/:id/pdf]', error);
