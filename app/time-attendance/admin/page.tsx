...
export default function TimeAttendanceAdminPage() {
  ...
  const handleExport = async (format: 'csv' | 'excel') => {
    ...
    try {
      const url = `/api/time-attendance/export?company_id=${companyId}&start_date=${selectedDate}&end_date=${selectedDate}&format=${format}`;
      ...
      if (res.ok) {
        ...
        a.href = downloadUrl;
        a.download = `presenze_${selectedDate}.${format === 'excel' ? 'xls' : 'csv'}`;
