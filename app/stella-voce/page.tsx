      ...
      <div style={{ padding: '8px 16px 30px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
        ...
        {showInput && (
          <form onSubmit={(e) => { e.preventDefault(); const t = typed.trim(); if (t) { sendText(t, true); setTyped(''); } }} style={{ display: 'flex', gap: 8, width: '100%', maxWidth: 460 }}>
            <input autoFocus value={typed} onChange={e => setTyped(e.target.value)} placeholder="Scrivi a Stella… (non risponde a voce)"
            ...
            <button type="submit" disabled={!typed.trim()} style={{ background: '#1f6feb', border: 'none', color: '#fff', borderRadius: '50%', width: 44, height: 44, fontSize: 18, cursor: 'pointer', flex: '0 0 auto' }}>➤</button>
            <button type="button" onClick={() => { setShowInput(false); setTyped(''); }} style={{ background: 'transparent', border: '1px solid rgba(120,160,255,.3)', color: '#9bb8ff', borderRadius: '50%', width: 44, height: 44, fontSize: 16, cursor: 'pointer', flex: '0 0 auto' }}>✕</button>
          </form>
        )}
