/**
 * URGENTLY RESTORE BLOG ARTICLES 76-80 with CORRECT approach
 *
 * THE FIX:
 * 1. Write Italian content WITHOUT context (sets base content)
 * 2. Then write translations WITH context for DE, FR, EN
 *
 * This is the opposite of what was done before (which only saved translations)
 */
...
<p>Die Schweiz hat spezifische Anforderungen für die Eröffnung eines Restaurants. Sie müssen verschiedene Lizenzen und Genehmigungen einholen.</p>
...
async function restoreArticle(article: any) {
  ...
  // STEP 1: Write Italian content WITHOUT context (this sets the BASE content)
  ...
  console.log(`   ✅ Italian base content saved`);
  ...
  console.log(`\n4️⃣  Writing English translation (WITH context: en_US)...`);
  ...
  if (!enSuccess) {
    console.log(`   ⚠️  WARNING: English translation failed`);
  } else {
...
async function main() {
  console.log('\n🚨 URGENT: RESTORING BLOG ARTICLES 76-80 🚨');
  console.log('='.repeat(60));
  console.log('CORRECT APPROACH:');
  console.log('1. Write Italian WITHOUT context (sets base)');
  console.log('2. Write DE/FR/EN WITH context (sets translations)');
  console.log('='.repeat(60));
  ...
  console.log('- Italian content as BASE');
  console.log('- German, French, English as TRANSLATIONS');
}
