...
// EXACT same function as in translate-content-english.ts
...
async function main() {
  console.log("Check exact search_read behavior...\n");
  ...
  for (let i = 0; i < Math.min(3, articles.length); i++) {
    ...
    console.log(`ID ${article.id} - ${article.name}`);
    console.log(`  Content length: ${article.content?.length || 0}`);
    console.log(`  Content: ${article.content?.substring(0, 200) || "[EMPTY]"}`);
    console.log();
  }
}
