...
def get_account_id(models, uid, konto_code):
    ...
    if not account_ids:
        print(f"    [!] Konto {konto_code} non trovato in Odoo")
        return None
...
def get_odoo_transactions(models, uid, account_id, date_from, date_to):
    ...
    if not move_line_ids:
        print(f"    [!] Nessuna transazione trovata")
        return []
...
def load_json_transactions(file_path, date_from, date_to):
    ...
    print(f"    [OK] Trovate {len(transactions)} transazioni in JSON")
    return transactions
...
def find_duplicates(odoo_lines):
    """Trova duplicati in Odoo (stessa data + importo)"""
    ...
    status = "[OK]" if not duplicates else "[!]"
    print(f"    {status} Trovati {len(duplicates)} gruppi di duplicati")
    return duplicates
...
def match_transactions(json_txs, odoo_lines):
    ...
    print(f"    {'[!]' if missing_in_odoo else '[OK]'} Mancanti in Odoo: {len(missing_in_odoo)}")
    print(f"    {'[!]' if extra_in_odoo else '[OK]'} Extra in Odoo: {len(extra_in_odoo)}")

    return matched, missing_in_odoo, extra_in_odoo

def analyze_account(models, uid, konto_code, konto_name, json_file, date_from, date_to):
    """Analizza un singolo konto"""
    print(f"\n{'='*80}")
    print(f"[*] ANALISI KONTO {konto_code} - {konto_name}")
    print(f"{'='*80}")
...
def print_summary(results):
    ...
    for res in results:
        ...
        print(f"    Duplicati: {res['summary']['duplicates_found']}")
        print()
...
def main():
    ...
    try:
        ...
        # Analizza konti
        accounts = [
            ("1025", "UBS EUR", "data-estratti/UBS-EUR-2024-TRANSACTIONS.json"),
            ("1024", "UBS CHF", None),  # Solo Odoo
        ]

        results = []
        for konto_code, konto_name, json_file in accounts:
            result = analyze_account(models, uid, konto_code, konto_name, json_file, date_from, date_to)
        ...
        for res in results:
            ...
            print(f"\n{'='*80}")
            print(f"[!] DUPLICATI IN {res['konto']} - {res['name']}")
            print(f"{'='*80}\n")

            for dup in res['duplicates']:
                print(f"Data: {dup['date']} | Importo: {dup['amount']:.2f} | Occorrenze: {dup['count']}")
        ...
        for res in results:
            ...
            for tx in res['missing_in_odoo'][:10]:
