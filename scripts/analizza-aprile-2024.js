...
// Utility: promisify xmlrpc calls
...
async function execute(uid, model, method, domain = [], fields = {}) {
 ...
 */
async function fetchApril2024Movements(uid) {
    ...
    for (const account of accounts) {
        ...
        // 1. Get account ID
        const accountRecords = await execute(uid, 'account.account', 'search_read', [
            [['code', '=', account.code]]
        ], {
        ...
        const moveLines = await execute(uid, 'account.move.line', 'search_read', [
            [
        ...
        const movements = moveLines.map((line, index) => {
        ...
        const openingLines = await execute(uid, 'account.move.line', 'search_read', [
            [
                ['account_id', '=', accountId],
                ['date', '<=', '2024-03-31'],
                ['parent_state', '=', 'posted']
 ...
 */
function crossCheckBankStatements(odooData) {
    console.log('\n=== CROSS-CHECK CON ESTRATTI BANCARI ===\n');

    // Load bank statement data
    let ubsChf, ubsEur, creditSuisse;
 ...
 */
function generateExecutiveSummary(odooData, comparison) {
    const summary = {
        period: 'Aprile 2024',
        date_from: '2024-04-01',
        date_to: '2024-04-30',
...
async function main() {
    ...
    try {
        ...
        // 2. Fetch movements
        const odooData = await fetchApril2024Movements(uid);
