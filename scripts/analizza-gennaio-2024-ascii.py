...
def get_odoo_transactions(models, uid, account_id, date_from, date_to):
    ...
    print(f"   Š Caricamento transazioni Odoo...")
...
def load_json_transactions(file_path, date_from, date_to):
    ...
    if not file_path:
        print(f"     Nessun file JSON disponibile (usa solo Odoo)")
        return []
...
def find_duplicates(odoo_lines):
    ...
    # Trova gruppi con pi di 1 transazione
    duplicates = []
    ...
    print(f"   {'' if not duplicates else ''}  Trovati {len(duplicates)} gruppi di duplicati")
    return duplicates
...
def match_transactions(json_txs, odoo_lines):
    ...
    print(f"   {'' if missing_in_odoo else ''}  Mancanti in Odoo: {len(missing_in_odoo)}")
    print(f"   {'' if extra_in_odoo else ''}  Extra in Odoo: {len(extra_in_odoo)}")
...
def analyze_account(models, uid, konto_code, date_from, date_to):
    ...
    print(f"\n{'='*80}")
    print(f"Š ANALISI KONTO {konto_code} - {KONTI[konto_code]['name']}")
    print(f"{'='*80}")
